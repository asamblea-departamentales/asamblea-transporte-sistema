...
<head>
    ...
    <style>
        body { font-family: 'DejaVu Sans', sans-serif; color: #333; line-height: 1.5; font-size: 11px; margin: 0; padding: 0; }
...
<body>
    ...
    <table class="info-table">
        ...
        <tr>
            ...
            <td class="value">{{ ucfirst($solicitud->tipo_solicitud ?? '—') }}</td>
            <td class="label">Tipo Mantenimiento:</td>
            <td class="value">{{ $solicitud->tipoMantenimiento?->nombre ?? '—' }}</td>
        </tr>
        <tr>
            <td class="label">Fecha sugerida:</td>
            <td class="value">{{ optional($solicitud->fecha_sugerida)->format('d/m/Y') ?? '—' }}</td>
            <td class="label">Fecha realizada:</td>
            <td class="value">{{ optional($solicitud->fecha_realizada)->format('d/m/Y') ?? 'Pendiente' }}</td>
        </tr>
    </table>

    {{-- DETALLE --}}
    <div class="section-title">Detalle del Servicio</div>
    <div class="obs-box" style="margin-bottom: 20px;">
        {{ $solicitud->detalle ?? 'Sin detalle registrado.' }}
    </div>
    ...
    <table class="totals-table">
        ...
        <tbody>
            <tr>
                ...
                <td style="text-align: right;">${{ number_format($solicitud->costo_estimado ?? 0, 2) }}</td>
            </tr>
            <tr class="bg-gray">
                <td>Costo Real (Facturas)</td>
                <td style="text-align: right;"><strong>${{ number_format($solicitud->costo_real ?? 0, 2) }}</strong></td>
            </tr>
            <tr class="bg-gray">
                <td>Monto Validado por Liquidador</td>
                <td style="text-align: right;"><strong>${{ number_format($liquidacion?->monto_validado ?? 0, 2) }}</strong></td>
            </tr>
            @php
                $diferencia = ($liquidacion?->monto_validado ?? 0) - ($solicitud->costo_real ?? 0);
            @endphp
            <tr>
