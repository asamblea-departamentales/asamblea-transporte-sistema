...
function RequestDetailRow({
  ...
  const isProgramada = String(request.estado).toUpperCase() === "PROGRAMADA";

  const handleCompleteClick = async () => {
    if (!confirm("¿Confirmas que el viaje ha finalizado y se ha completado?"))
      return;
    ...
    try {
      ...
      console.error(error);
      alert(error?.message ?? "Error al completar la solicitud.");
    } finally {
      setIsUpdating(false);
    }
  };
              ...
              <div>
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <p>
                    ...
                    {formatFecha(request.fecha_retorno ?? undefined)}
                  </p>
                </div>
              </div>
            </div>
...
export default function RecentRequestsWidget() {
  ...
  const handleCompleteRequest = async (id: number) => {
    // ✅ manda payload mínimo (ajusta si tu backend valida otros campos)
    await completeRequest(id, { observacion: "Finalizada desde dashboard" });
    ...
    // ✅ actualizar detalle
    setExpandedRequest((cur) =>
      cur && cur.id === id ? { ...cur, estado: "COMPLETADA" } : cur
    );
  };
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <span className="text-sm font-medium text-slate-600">
                        {formatFechaTabla(req.fecha_salida)}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4 text-right">
                      <span
                        className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-bold capitalize ${getStatusStyles(
                          req.estado
                        )}`}
                      >
                        {(String(req.estado).toLowerCase() === "completada" ||
                          String(req.estado).toLowerCase() === "finalizada") && (
                          <svg
                        ...
                        {String(req.estado).toLowerCase()}
                      </span>
                    </td>
                  </tr>

                  {expandedId === req.id && expandedRequest && (
                    <RequestDetailRow request={expandedRequest} onComplete={handleCompleteRequest} />
                  )}
                </Fragment>
              ))
            )}
