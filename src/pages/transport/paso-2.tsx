...
// Iconos de mapa creados UNA vez — no dentro de efectos ni renders
...
function MapPanel({ origenCoords, origenLabel, destinos }: MapPanelProps) {
  // Ref al elemento DOM — evita el ID hardcodeado que rompe en StrictMode
  ...
  useEffect(() => {
    ...
    // Emitir evento custom al hacer click en el mapa
  ...
  // Actualizar marcadores y ruta cuando cambian las coords
  useEffect(() => {
  ...
  return (
    <div className="sticky top-24 space-y-4">
      <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        ...
        {/* Ref en el div del mapa — no ID hardcodeado */}
        <div ref={mapContainerRef} className="h-[420px] bg-slate-50" />
...
export default function TransportStep2Page() {
  ...
  const origenCoordsRef = useRef<{ lat: number; lng: number } | null>(null);
  ...
  useEffect(() => { origenRef.current       = origen;       }, [origen]);
  useEffect(() => { origenCoordsRef.current = origenCoords; }, [origenCoords]);
  ...
  // Usa refs para leer estado actual — evita setState dentro de setState
  useEffect(() => {
    function onMapClick(e: CustomEvent<{ lat: number; lng: number }>) {
      const { lat, lng } = e.detail;

      // reverseGeocode usa el proxy de Vercel (/nominatim/reverse) — sin fetch directo
      reverseGeocode(lat, lng)
        .then(display_name => {
          const address = display_name ?? `${lat.toFixed(6)}, ${lng.toFixed(6)}`;

          if (!origenCoordsRef.current) {
            setOrigen(address);
            ...
            setDestinos(ds => {
              ...
              if (idx !== -1) {
                return ds.map((d, i) => i === idx ? { ...d, address, lat, lng } : d);
              }
              return [...ds, { id: uid(), address, lat, lng }];
            });
          }
        })
        .catch(err => console.error("[map:click] reverseGeocode:", err));
    }
  ...
  }, []); // deps vacías — solo se registra una vez, lee estado via refs
  ...
  function removeDestino(id: string) {
    if (destinos.length > 1) setDestinos(ds => ds.filter(d => d.id !== id));
  }
          ...
          <div className="space-y-8 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
                ...
                {destinos.map((dest, idx) => (
                  <div key={dest.id}>
                    <div className="mb-2 flex items-center justify-between">
                      ...
                      {destinos.length > 1 && (
                        <button type="button" onClick={() => removeDestino(dest.id)}
            ...
            <div className="flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <button onClick={handleContinue}
                className="inline-flex items-center justify-center gap-2 rounded-2xl bg-indigo-600 px-6 py-3 text-sm font-bold text-white shadow-lg shadow-indigo-500/20 transition hover:bg-indigo-700 active:scale-[0.99]">
                Continuar
                <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
